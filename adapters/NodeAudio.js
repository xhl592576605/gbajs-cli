/**
 * Node.js音频处理适配器
 * 适配GBA音频处理模块到Node.js环境
 */
const Speaker = require('speaker');
const AudioHandler = require('../interfaces/AudioHandler');
...
function GameBoyAdvanceAudio() {
  ...
  try {
    ...
    console.warn('Failed to initialize audio output:', e);
    this.speaker = null;
  }
...
GameBoyAdvanceAudio.prototype.clear = function() {
  ...
  if (this.speaker) {
    this.resampleRatio = this.sampleRate / this.speaker.sampleRate;
  }
...
/**
 * 设置Node.js音频处理器
 * @param {AudioHandler} audioHandler - Node.js音频处理器实例
 */
GameBoyAdvanceAudio.prototype.setAudioHandler = function(audioHandler) {
  this.audioHandler = audioHandler;

  // 初始化音频处理器
  if (this.audioHandler) {
    this.audioHandler.initialize(this.sampleRate);
  }
};
...
GameBoyAdvanceAudio.prototype.sample = function() {
  ...
  // 在Node.js环境中，将音频样本推送到音频处理器
  if (this.audioHandler) {
    this.audioHandler.pushSample(sampleLeft, sampleRight);
  }

  // 如果有speaker，也写入到speaker
  if (this.speaker) {
    // 将浮点样本转换为16位整数
    const leftInt = Math.round(sampleLeft * 32767);
    const rightInt = Math.round(sampleRight * 32767);

    // 创建缓冲区并写入数据
    const buffer = Buffer.alloc(4);
    buffer.writeInt16LE(leftInt, 0);
    buffer.writeInt16LE(rightInt, 2);
    this.speaker.write(buffer);
  }
};

GameBoyAdvanceAudio.prototype.audioProcess = function(audioProcessingEvent) {
  // 在Node.js环境中，这个方法可能不需要
  // 音频处理已经在sample方法中完成
};
